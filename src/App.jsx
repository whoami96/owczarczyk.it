    ...
    <div className="min-h-screen bg-slate-900 text-slate-300 selection:bg-emerald-500/30">
      ...
      <header className="container mx-auto px-6 pt-24 pb-16 text-center">
        <div className="h-12 mb-8 overflow-hidden">
          <div key={activeBadge} className="animate-badge inline-block p-2 px-4 rounded-full bg-emerald-500/10 border border-emerald-500/20">
        ...
        <p className="text-xl md:text-2xl text-slate-400 max-w-3xl mx-auto leading-relaxed mb-10">
          ...
          <span className="block mt-4 text-slate-500 text-lg md:text-xl font-light italic">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <a href="mailto:kontakt@owczarczyk.it" className="flex items-center gap-2 px-8 py-4 bg-emerald-500 text-slate-900 rounded-xl font-bold hover:bg-emerald-400 transition-all shadow-lg shadow-emerald-500/20">
            Porozmawiajmy o projekcie
            <ArrowRight className="w-5 h-5" />
          </a>
          <a href="https://github.com/whoami96" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-8 py-4 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-700 transition-all border border-slate-700">
      ...
      <div className="border-y border-slate-800 bg-slate-900/50 py-10 overflow-hidden relative">
        <div className="animate-marquee flex whitespace-nowrap">
          {[...techStack, ...techStack].map((tech, i) => (
            <span key={i} className="text-2xl font-bold font-mono tracking-tighter text-slate-500 mx-12 hover:text-emerald-500 transition-colors cursor-default">
              {tech}
            </span>
          ))}
        </div>
      ...
      <section className="container mx-auto px-6 py-24">
        <div className="flex items-center gap-4 mb-16">
          <h2 className="text-3xl font-bold text-white tracking-tight">Jak mogę pomóc?</h2>
          <div className="h-px flex-1 bg-slate-800"></div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
          {services.map((service, index) => (
            <div key={index} className="group p-8 rounded-2xl bg-slate-800/40 border border-slate-700/50 hover:border-emerald-500/30 transition-all duration-300 hover:shadow-2xl hover:shadow-emerald-500/5">
              ...
              <div className="flex flex-wrap gap-2">
                {service.tags.map((tag, tIdx) => (
                  <span key={tIdx} className="px-3 py-1 text-xs font-mono bg-slate-900/80 text-emerald-500/80 rounded-full border border-slate-700">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="p-8 md:p-12 rounded-3xl bg-emerald-500/5 border border-emerald-500/10 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <Terminal className="w-32 h-32 text-emerald-500" />
            </div>
            <h3 className="text-xl font-bold text-emerald-500 mb-6 font-mono uppercase tracking-widest">Dlaczego on-premise?</h3>
            <blockquote className="text-2xl md:text-3xl text-white font-light leading-snug italic">
      ...
      <section id="contact" className="container mx-auto px-6 py-24 mb-12">
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl p-12 text-center text-white shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 -mr-20 -mt-20 w-64 h-64 bg-white/10 rounded-full blur-3xl"></div>
          <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-64 h-64 bg-black/10 rounded-full blur-3xl"></div>

          <h2 className="text-4xl font-bold mb-6 relative z-10 text-slate-900">Zacznijmy współpracę</h2>
          <p className="text-emerald-950/80 text-lg mb-10 max-w-xl mx-auto relative z-10 font-medium">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 relative z-10 mb-10">
            <a href="mailto:kontakt@owczarczyk.it" className="flex items-center gap-3 px-8 py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-black transition-colors shadow-xl w-full sm:w-auto justify-center">
              <Mail className="w-5 h-5" />
              kontakt@owczarczyk.it
            </a>
            <a href="https://www.linkedin.com/in/pawel-owczarczyk/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-8 py-4 bg-white/10 text-white backdrop-blur-sm border border-white/20 rounded-xl font-bold hover:bg-white/20 transition-all w-full sm:w-auto justify-center">
              <LinkedInIcon className="w-5 h-5" />
              LinkedIn
            </a>
            <a href="https://github.com/whoami96" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-8 py-4 bg-white/10 text-white backdrop-blur-sm border border-white/20 rounded-xl font-bold hover:bg-white/20 transition-all w-full sm:w-auto justify-center">
              <GitHubIcon className="w-5 h-5" />
              GitHub
            </a>
          </div>

          <div className="flex flex-wrap justify-center gap-8 text-emerald-950/70 font-mono text-sm relative z-10">
      ...
      {/* Footer */}
      <footer className="container mx-auto px-6 py-12 border-t border-slate-800 text-center text-slate-500 text-sm font-mono">
